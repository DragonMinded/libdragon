...
#define BUFFER_INIT(value, bits) \
    u32 bits = 32; \
    u32 value = h264bsdShowBits32(pStrmData);
...
// N64 has a 64-bit processor so we can locally cache a 64-bit value
// from which bits are extracted.

/* macro to initialize stream buffer cache, fills the buffer (32 bits) */
#define BUFFER_INIT(value, bits) \
    u32 bits; \
    u64 value = h264bsdShowBits64(pStrmData, &bits);
...
{ \
    if (bits < (numBits)) \
    { \
        if(h264bsdFlushBits(pStrmData,64-bits) == END_OF_STREAM) \
            return(HANTRO_NOK); \
        value = h264bsdShowBits64(pStrmData, &bits); \
    } \
    (outVal) = value >> (64 - (numBits)); \
}
...
#define BUFFER_GET(value, bits, outVal, numBits) \
{ \
    BUFFER_SHOW(value, bits, outVal, numBits); \
    BUFFER_FLUSH(value, bits, numBits); \
}

#define BUFFER_CLOSE(bits) \
{ \
    if (h264bsdFlushBits(pStrmData, 64-bufferBits) != HANTRO_OK) \
        return(HANTRO_NOK); \
}
...
/* Code */
    ...
    if (totalCoeff != 0)
    {
        ...
        i16 *clevel = &level[totalCoeff-1];
        u8 idx = zerosLeft;
        if (maxNumCoeff == 15)
            idx++;

        int dozigzag = (maxNumCoeff > 4);
        static const u8 zigzag[16] = {
            0, 1, 4, 8, 5, 2, 3, 6, 9, 12, 13, 10, 7, 11, 14, 15
        };

        levelSuffix = 0;

        // Iterate through all coefficients. Arrange the loop so that
        // we iterate the last time with i==-1
        int i = totalCoeff-2;
        do {
            i32 lvl = *clevel--;
            u8 idx2 = idx;
            if (dozigzag)
                idx2 = zigzag[idx];
            levelSuffix |= 1<<idx2;
            // If it is the last coefficient, we must set the
            // end flag (0x20). Given that we made it so i == -1 in the last
            // loop, just copy the 0x20 bit.
            idx2 |= i & 0x20;        
            *buf++ = idx2;
            *buf++ = lvl;
            if ((i8)lvl != lvl) {
                *buf++ = lvl>>8;
                buf[-3] |= 0x10;     // 16-bit flag 0x10
            }
            if (i >= 0)
                idx += *crun--;
        } while (i-- >= 0);
