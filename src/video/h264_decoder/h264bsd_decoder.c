...
/* Code */
    ...
    if (pStorage->prevBufNotFinished && byteStrm == pStorage->prevBufPointer)
    {
        ...
        #ifdef H264BSD_N64
        // rasky: FIXME: is this really required?
        strm.pCurr = (u64)(u32)strm.pStart << 3;
        #else
    ...
    {
        #ifndef H264BSD_N64
        // Deblocking filter currently not supported on N64. Even if it's
        // disabled during encoding, it's useless to go through all macroblocks
        // and find out that there's nothing to do, so let's just skip the
        // call altogether.
