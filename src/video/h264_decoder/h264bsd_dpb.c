...
/* Code */
    ...
    /* pictures in the output buffer are not available for processing,
       put them at the beginning of the array. */
    if (pic1->inOutputBuf && !pic2->inOutputBuf)
        return(-1);
    else if (!pic1->inOutputBuf && pic2->inOutputBuf)
        return(1);
...
/* Code */
    ...
    ASSERT(dpb->currentOut->inOutputBuf == 0);
...
/* Code */
    ...
    /* Record that this picture is currently in the output buffer and cannot
...
/*------------------------------------------------------------------------------
    ...
    Function: h264bsdDpbNumOutputPictures

        Functional description:
            Function to get the number of pictures that are available in the
            output buffer.

        Return:
            Number of pictures that are ready to be displayed.
