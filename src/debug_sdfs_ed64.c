...
// If timed out, we always return 0xFF and set the start bit to 1 as well
#define ED64_SD_ACMD41_TOUT_MS          1000
#define ED64_SD_ACMD41_WAIT_MS            10
#define ED64_SD_TIMEOUT                 0x80
...
static uint32_t everdrive_sd_active_mode = 0;
...
static uint32_t __attribute__((aligned(16))) everdrive_sd_config = 0;
...
uint8_t everdrive_sd_read_first() {
	uint8_t res;
	uint32_t timeout = 64; // 8 bytes * 8 bits
...
static uint8_t everdrive_sd_resp_buff[5] = { 0 };
// Wait for and read an RX like response
static uint8_t everdrive_sd_read_RX() {
	uint8_t timeout = 16;
	everdrive_sd_resp_buff[0] = everdrive_sd_read_first();

	// read remaining bytes
	set_everdrive_sd_bitlen(8);
	everdrive_sd_resp_buff[1] = everdrive_sd_read_command();
	everdrive_sd_resp_buff[2] = everdrive_sd_read_command();
	everdrive_sd_resp_buff[3] = everdrive_sd_read_command();
	everdrive_sd_resp_buff[4] = everdrive_sd_read_command();

	// Make sure everything is consumed, we just don't use them
	while(everdrive_sd_read_command() != 0xFF) {
		if (!timeout--) return 0xFF;
	};
	return everdrive_sd_resp_buff[0];
}

static uint8_t everdrive_sd_execute_command(uint8_t cmd, uint32_t arg) {
	...
	uint8_t result = everdrive_sd_read_RX();
	if (result & ED64_SD_TIMEOUT) {
...
static uint8_t everdrive_sd_send_app_command(uint8_t cmd, uint32_t rca, uint32_t arg) {
	uint8_t result;
	// Next command will be an application specific cmd
	result = everdrive_sd_execute_command(ED64_SD_CMD55, rca);
	if (result & ED64_SD_TIMEOUT) {
		debugf("ACMD%u CMD55 err\n", cmd & ~0x40);
		return result;
	};

	result = everdrive_sd_execute_command(cmd, arg);
...
static uint8_t everdrive_sd_stop_transmission() {
	uint8_t result;
	uint16_t timeout = -1;

	if ((everdrive_sd_active_mode & ED64_SD_MODE_ACCESS) == ED64_SD_MODE_NONE) return 0;

	result = everdrive_sd_execute_command(ED64_SD_CMD12, 0);
	if (result & ED64_SD_TIMEOUT) return result;
...
static uint8_t everdrive_sd_change_mode(uint32_t mode, LBA_t addr) {
	uint8_t result;
	if (
	...
	result = everdrive_sd_stop_transmission();
	if (result & ED64_SD_TIMEOUT) return result;

	result = everdrive_sd_execute_command(
...
static DSTATUS fat_disk_initialize_everdrive(void) {
	...
	// Set lo speed for initialization
	everdrive_sd_config &= ~ED64_SD_CFG_SPEED;
	io_write(ED64_BASE_ADDRESS + ED64_REGISTER_SD_STATUS, everdrive_sd_config);

	// Put in idle
	everdrive_sd_execute_command(ED64_SD_CMD0, 0);

	// IF cond
	result = everdrive_sd_execute_command(ED64_SD_CMD8, 0x1AA);

	if(result & ED64_SD_TIMEOUT) return RES_ERROR;

	if (everdrive_sd_resp_buff[3] != 1) {
		debugf("SD card - voltage mismatch\n");
		return RES_ERROR;
	}

	if(everdrive_sd_resp_buff[4] != 0xAA) {
		debugf("SD card did not echo\n");
		return RES_ERROR;
	};
	...
	while(1) {
		...
		result = everdrive_sd_send_app_command(ED64_SD_CMD41, 0, 0x40300000);

		// Check ready bit on OCR
		if ((everdrive_sd_resp_buff[1] & 0x80) != 0) break;

		wait_ms(ED64_SD_ACMD41_WAIT_MS);
	};

	// Check CCS and set HC mode
	everdrive_sd_active_mode |= everdrive_sd_resp_buff[1] & 0x40;

	result = everdrive_sd_execute_command(ED64_SD_CMD2, 0);
	if (result & ED64_SD_TIMEOUT) return RES_ERROR;

	result = everdrive_sd_execute_command(ED64_SD_CMD3, 0);
	if (result & ED64_SD_TIMEOUT) return RES_ERROR;

	sd_rca = 	(everdrive_sd_resp_buff[1] << 24) |
				(everdrive_sd_resp_buff[2] << 16) |
				(everdrive_sd_resp_buff[3] << 8) |
				everdrive_sd_resp_buff[4];

	result = everdrive_sd_execute_command(ED64_SD_CMD7, sd_rca);
	if (result & ED64_SD_TIMEOUT) return RES_ERROR;

	// Set bus width to 4
	result = everdrive_sd_send_app_command(ED64_SD_CMD6, sd_rca, 0x2);
	if (result & ED64_SD_TIMEOUT) {
		debugf("ACMD6 err\n");
		return RES_ERROR;
	};
...
static DRESULT fat_disk_read_everdrive(BYTE* buff, LBA_t sector, UINT count)
{
	...
	PI_regs->dom1_pulse_width = 0x09;

	if(everdrive_sd_change_mode(ED64_SD_MODE_BLOCK_READ, sector)) {
		PI_regs->dom1_pulse_width = old_pw;
		return RES_ERROR;
	};
