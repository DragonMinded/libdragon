...
typedef struct {
    ...
    void *report_ctx;            ///< Context to pass to the report callback
} fsck_ctx_t;
...
static int fsck_fsid(fsck_ctx_t *ctx, cpakfs_id_t *id)
{
    ...
    for (int i=0; i<4; i++) {
        ...
        if (checksum1 == 0x0101 && be16(sectors[i].checksum1) == 0x0101 && be16(sectors[i].checksum2) == 0xFEFD) {
            valid = &sectors[i];
            if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_FSID_DEXDRIVE, CPAKFS_LEVEL_INFO,
    ...
    if (valid == NULL) {
        ...
        for (int i=0; i<4; i++) {
            if (memcmp(valid->data8, sectors[i].data8, BLOCK_SIZE) != 0) {
                ...
                if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_FSID_CORRUPTED_BACKUP, CPAKFS_LEVEL_INFO,
    ...
    if (ctx->mode >= MODE_CHECK && (be16(valid->device_id_lsb) & 0x01) != 0x01) {
        if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_FSID_WRONG_DEVICE_ID, CPAKFS_LEVEL_INFO,
...
static int fsck_fat(fsck_ctx_t *ctx, cpakfs_id_t* fsid, cpakfs_fat_entry_t **out_fat)
{
    ...
    for (int i=0; i<fat_size; i+=PAGE_SIZE) {
        ...
        if (!main_csum_ok && !backup_csum_ok) {
            ctx->nissues++;
            if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_FAT_CHECKSUM_FAILURE, CPAKFS_LEVEL_ERROR,
            ...
            if (memcmp(fat + i/2, backup, PAGE_SIZE) != 0) {
                if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_FAT_UNSYNCHRONIZED, CPAKFS_LEVEL_INFO,
        ...
        for (int i=0; i < fat_size/2; i++) {
            ...
            if (i < reserved) {
                ...
                if (!FAT_IS_RESERVED(fat[i])) {
                    if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_FAT_INVALID_RESERVED, CPAKFS_LEVEL_INFO,
            ...
            // we need to report it as an error.
                ...
                ctx->nissues++;
                if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_FAT_INVALID_RESERVED, CPAKFS_LEVEL_ERROR,
            ...
            if (FAT_IS_VALID(fat[i], reserved) && fat[i].bank >= be16(fsid->bank_size_msb) >> 8) {
                ctx->nissues++;
                if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_FAT_INVALID_BANK, CPAKFS_LEVEL_ERROR,
...
static int fsck_chains(fsck_ctx_t *ctx, int num_banks, cpakfs_fat_entry_t fat[][NUM_PAGES], cpakfs_fat_entry_t roots[MAX_NOTES], int nroots)
{
    ...
    for (int i = 0; i < nroots; i++) {
        ...
        while (true) {
            ...
            if (BIT_TEST(visited, current)) {
                ctx->nissues++;
                if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_CHAIN_INFINITE_LOOP, CPAKFS_LEVEL_ERROR,
            ...
            if (!FAT_IS_VALID(next, reserved)) {
                ctx->nissues++;
                if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_CHAIN_NO_TERMINATOR, CPAKFS_LEVEL_ERROR,
        ...
        for (int page = reserved; page < NUM_PAGES; page++) {
            ...
            while (true) {
                if (BIT_TEST(processed, current)) {
                    ...
                    ctx->nissues++;
                    if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_CHAIN_COLLISION, CPAKFS_LEVEL_ERROR,
    ...
    for (int i = 0; i < num_orphan_roots; i++) {
        if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_CHAIN_ORPHANED, CPAKFS_LEVEL_WARNING,
...
static int fsck_notes(fsck_ctx_t *ctx, cpakfs_id_t* fsid, cpakfs_fat_entry_t *fat)
{
    ...
    for (int i=0; i<MAX_NOTES; i++) {
        ...
        if (occupied &&
            ...
            ctx->nissues++;
            if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_NOTE_INVALID_GAMECODE, CPAKFS_LEVEL_ERROR,
        ...
        if (occupied &&
            ...
            ctx->nissues++;
            if (ctx->report) ctx->report(ctx->report_ctx, CPAKFS_ISSUE_NOTE_INVALID_PUBCODE, CPAKFS_LEVEL_ERROR,
...
int cpakfs_fsck(joypad_port_t port, bool fix_errors, cpakfs_report_fn report, void *report_ctx)
{
    int err;
