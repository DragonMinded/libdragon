...
typedef struct uso_sym_s {
    ...
    uint16_t section;	///< Source section of export symbol
    uint8_t flags;		///< Detects if a symbol is weak
...
typedef struct uso_module_s {
	...
	uint16_t num_sections;		///< Section count
	uint16_t eh_frame_section;	///< .eh_frame section index
	uint16_t ctors_section;		///< .ctors section index
	uint16_t dtors_section;		///< .dtors section index
} uso_module_t;
