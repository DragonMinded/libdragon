...
inline void __rdpq_font_glyph_metrics(const rdpq_font_t *fnt, int16_t index, float *xadvance, int8_t *xoff2, bool *has_kerning, uint8_t *sort_key)
{
    glyph_t *g = &fnt->glyphs[index];
    *xadvance = g->xadvance * (1.0f / 64.0f);
    *xoff2 = g->xoff2;
    *has_kerning = g->kerning_lo != 0;
    *sort_key = g->natlas;
}
