...
// Known/supported format versions. Must be kept in sync with SYMT_VERSION in n64sym.c.
...
int main(int argc, char **argv) {
    ...
    // actually supported (see the byte-order check in rompak_find_ext).
    const char *ext = strrchr(path, '.');
    if (ext && (strcasecmp(ext, ".z64") == 0 ||
                strcasecmp(ext, ".n64") == 0 ||
                strcasecmp(ext, ".v64") == 0)) {
        g_symt_base = rompak_find_ext(f, ".sym");
        if (g_symt_base < 0)
            return 1;
    }
