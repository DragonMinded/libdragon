...
void elf_section_collect_uso(elf_info_t *elf_info)
{
	...
	//Make sure that too many sections haven't been pushed
	assert(arrlenu(elf_info->uso_src_sections) <= UINT16_MAX);
}
...
void uso_sym_table_build(uso_sym_table_t *sym_table, elf_symbol_t **elf_symbols)
{
	for(size_t i=0; i<arrlenu(elf_symbols); i++) {
		...
		symbol.section = elf_symbols[i]->section;
		symbol.flags = 0;
		//Mark symbol as weak
...
void uso_module_set_section_id(elf_info_t *elf_info, char *name, uint16_t *dst)
{
	size_t section_id = 0;
...
uso_file_sym_t uso_generate_file_sym(uso_sym_t *sym)
{
	...
	temp.section = sym->section;
	temp.flags = sym->flags;
	temp.__padding = sym->__padding;
	return temp;
}
...
void uso_write_file_sym(uso_file_sym_t *file_sym, uint32_t offset, FILE *out)
{
	...
	w16(out, file_sym->section);
	w8(out, file_sym->flags);
	w8(out, file_sym->__padding);
}
...
void uso_write_file_module(uso_file_module_t *file_module, uint32_t offset, FILE *out)
{
	...
	w16(out, file_module->num_sections);
	w16(out, file_module->eh_frame_section);
	w16(out, file_module->ctors_section);
